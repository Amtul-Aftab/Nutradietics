...
6. WHERE multiple AI API keys are configured (`AI_API_KEY`, `AI_API_KEY_2`, `AI_API_KEY_3`) THEN the system SHALL round-robin the starting key across calls — each new AI call SHALL begin with a different configured key — so that daily quota usage spreads evenly across the keys rather than always hitting one key first. WHEN an AI call fails specifically with a rate-limit error (HTTP 429) on the first-tried key THEN the system SHALL fall through the remaining configured keys in order until one succeeds, before returning a retryable error, so that no single key's daily quota becomes a hard ceiling. WHERE only one key is configured, OR the failure is not a rate-limit error, OR every tried key is rate-limited THEN the system SHALL return the existing retryable error unchanged (a non-429 failure fails fast without trying other keys). This failover SHALL remain internal to the AI adapter and transparent to callers (the four `AiClient` methods).
